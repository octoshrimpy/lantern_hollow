export function bindControls({ swipePad, btnA, btnB, onInput }) {
  const HOLD_MS = 180;
  const THRESHOLD = 18;
  let startX = 0; let startY = 0; let active = null; let timer = null; let moving = false;

  const emit = (key, source) => onInput({ key, source });
  const stop = () => { if (timer) clearInterval(timer); timer = null; active = null; moving = false; };
  const dirFrom = (dx, dy) => {
  ...
  swipePad.addEventListener('pointerdown', (e) => {
    moving = true; startX = e.clientX; startY = e.clientY; swipePad.setPointerCapture(e.pointerId);
  });
  ...
  swipePad.addEventListener('pointerup', stop);
  swipePad.addEventListener('pointercancel', stop);

  const bindButton = (button, key, label) => {
    button.addEventListener('pointerdown', () => { button.classList.add('active'); emit(key, label); });
